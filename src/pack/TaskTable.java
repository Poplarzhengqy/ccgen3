...
*/
		...
		menu.add(new SetDateTimeAction(this, selectedViewRows, new MActionInfo(TaskState.TODAY.toString()), TaskState.TODAY));
		...
		MMenu nextMenu = new MMenu(_("Next"));
	...
	public TaskSelection getTaskSelection() {
		...
		for (int i : selectedRows)
			taskSelection.addTaskCopy(getTaskAt(convertRowIndexToModel(i)));
...
// TODO: popup menu instead of dialog
			Color initialColor = (singleSelection != null) ? singleSelection.getColor() : null;//!!!undo
	...
	public static class SetNextDateAction extends SetValueAction<Object> {
		...
		public SetNextDateAction(final TaskTable table, final int[] selectedViewRows) {
			super(table, selectedViewRows, new MActionInfo(_("Apply Repeat Rule"), "ui/refresh"));
	...
	public static class SetRepeatRuleAction extends SetValueAction<RepeatRule> {
		...
		public void onAction() {
			...
			if (selection.length == 0) {
				...
				System.err.println("SELECTION: " + java.util.Arrays.toString(selection));//!!!!
			...
			System.err.println("USER RULE: " + repeatRule);//!!!!
			...
			for (int i : selection) {
				...
				repeatRule.set(task);
				model.fireTableRowsUpdated(modelRow, modelRow);
				System.err.println("SET RULE: " + task + " -> " + repeatRule);//!!!!
			}
		}
